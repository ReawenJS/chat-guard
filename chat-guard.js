...
.addField('⚔️ - Komutlar Ve Kullanımları',`
  ...
  **Etiket koruma** = __${config.prefix}etiketkoruma aç/kapat__
  **Mesaj limit** = __${config.prefix}mesajlimit aç/kapat__

Hangi korumanın aktif olduğunu görmek için ${config.prefix}korumalar yazabilirsin.
...
message.channel.send(komutlarembed)
  ...
  const ayarlarembed = new MessageEmbed()
.setColor('RANDOM')
.setAuthor(message.member.user.username, message.member.user.displayAvatarURL({dynamic: true, format: "png", size: 1024}))
.addField('⚔️ - Korumalar',`
  ...
  **Etiket koruma**: ${etiketaktif}
  **Mesaj limit**: ${karakteraktif}
 `)
.setTimestamp()
.setFooter(message.guild.name, message.guild.iconURL({dynamic: true, format: "png", size: 1024}))
